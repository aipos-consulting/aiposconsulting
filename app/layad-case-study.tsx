...
export default function LayadCaseStudy({ lang }: { lang: Lang }) {
  ...
  const cards = [[c.challengeTitle, c.challenge], [c.solutionTitle, c.solution], [c.resultTitle, c.result], [c.gateTitle, c.gate]] as const
        ...
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          ...
          <div>
            <div className="grid gap-5 sm:grid-cols-2">
              {cards.map(([title, description], index) => { const Icon = icons[index]; return (
                <article key={title} className="rounded-3xl border border-border bg-surface/50 p-6 shadow-sm md:p-7">
                  ...
                  <h3 className="mt-5 text-lg font-bold text-navy md:text-xl">{title}</h3><p className="mt-3 text-sm leading-7 text-muted">{description}</p>
                </article>
              )})}
            </div>
            <div className="mt-6 rounded-3xl border border-teal/30 bg-teal/[0.05] p-6 md:p-8">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-2">{c.process.map((step, index) => <div key={step} className="flex items-center gap-2"><span className="rounded-full bg-white px-3 py-2 text-xs font-bold text-navy shadow-sm ring-1 ring-border">{step}</span>{index < c.process.length - 1 && <ArrowRight size={14} className="text-teal" />}</div>)}</div>
              <div className="mt-6 flex items-start gap-3 border-t border-teal/20 pt-5 text-sm leading-6 text-navy/80"><CheckCircle2 size={18} className="mt-0.5 shrink-0 text-teal" /><span>{c.gate}</span></div>
            </div>
          </div>
        </div>
      </div>
